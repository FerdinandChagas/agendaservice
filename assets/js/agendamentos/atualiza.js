...
function handleButtonClick(event) {
    ...
    if (token) {
        // Determina a ação com base no tipo de botão (concluir ou cancelar)
        let status = (button.id === 'concluir') ? 'Concluído' : 'Cancelado';

        // Fazendo uma solicitação PATCH para a API de agendamentos
        fetch('http://localhost:8000/api/agendamentos/fila/' + data_id +'/', {
            method: 'PATCH',
            headers: {
...
document.getElementById('dados').addEventListener('click', function(event) {
    if (event.target.tagName.toLowerCase() === 'i' && (event.target.classList.contains('bi-check2-square') || event.target.classList.contains('bi-backspace-reverse-fill'))) {
